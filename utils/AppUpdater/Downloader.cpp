...
void Downloader::startDownload (const QUrl& url)
{
    ...
    bool tr = QFile::remove (m_downloadDir.filePath (m_fileName));
    tr = QFile::remove (m_downloadDir.filePath (m_fileName + PARTIAL_DOWN));
    tr = QFile::remove (m_downloadDir.filePath (m_fileName + "old"));
...
void Downloader::finished()
{
    ...
    /* Rename file */
    bool flag = QFile::rename (m_downloadDir.filePath (m_fileName),
                               m_downloadDir.filePath (m_fileName+"old"));


    flag = QFile::rename (m_downloadDir.filePath (m_fileName + PARTIAL_DOWN),
                               m_downloadDir.filePath (m_fileName));
//    if(flag == false){
//        QMessageBox box; box.setText("Could not rename file : "+m_downloadDir.filePath (m_fileName)
//                                     + " from : " + m_downloadDir.filePath (m_fileName + PARTIAL_DOWN)
//                                     ); box.exec();
//    }
//    else{
//        QMessageBox box; box.setText("Renamed file to "+m_downloadDir.filePath (m_fileName)); box.exec();
//    }
    /* Notify application */
...
void Downloader::installUpdate()
{
    ...
    if (box.exec() == QMessageBox::Ok) {
        if (!useCustomInstallProcedures())
        { QMessageBox box; box.setText("Open Download"); box.exec();
            openDownload();}
    }
    /* Wait */
    else {
        if(m_mandatoryUpdate)
        {QMessageBox box; box.setText("Quit app"); box.exec();
            QApplication::quit();}
