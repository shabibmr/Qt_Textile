#include "gmreportsortfilterproxymodel.h"
#include <QDebug>
GMReportSortFilterProxyModel::GMReportSortFilterProxyModel(QObject *parent)
    : QSortFilterProxyModel(parent)
{
//    qDebug()<<Q_FUNC_INFO;
//    this->avlCols = avlCols;
}

void GMReportSortFilterProxyModel::setStr(const QString &value)
{
    str = value;
}

void GMReportSortFilterProxyModel::setAvlCols(const QList<int> &value)
{
    avlCols = value;
//    qDebug()<<Q_FUNC_INFO<<__LINE__<<avlCols<<value;
}

bool GMReportSortFilterProxyModel::filterAcceptsRow(int sourceRow, const QModelIndex &sourceParent) const
{
...
//        qDebug()<<Q_FUNC_INFO<<sourceModel()->data(index).toString().contains(str)<<str<<sourceModel()->data(index).toString();
        if(sourceModel()->data(index).toString().contains(filterRegExp()) ||
                sourceModel()->data(index).toString().contains(str) ){
//            qDebug()<<"found match at "<<index.row()<<index.column();
            return true;
        }
    }
//    qDebug()<<Q_FUNC_INFO<<"no match"<<indexList.size();
    return false;


}

bool GMReportSortFilterProxyModel::lessThan(const QModelIndex &source_left, const QModelIndex &source_right) const
{
    //    qDebug()<<Q_FUNC_INFO;
...
QVariant GMReportSortFilterProxyModel::headerData(int section, Qt::Orientation orientation, int role) const
{
